...
namespace analysis2
{
    ...
    public class Histogram
    {
        ...
        static readonly Pen[] p = new Pen[] { Pens.Red, Pens.Green, Pens.Blue, Pens.Black };
        ...
        public Bitmap VisualizeGray()
        {
            int oneColorHeight = 100;
            int margin = 10;

            float[] maxValues = new float[] { Gray.Max() };
            byte[][] values = new byte[][] { Gray };


            Bitmap histogramBitmap = new Bitmap(800, oneColorHeight * 3 + margin * 4);
            Graphics g = Graphics.FromImage(histogramBitmap);
            g.FillRectangle(Brushes.White, 0, 0, histogramBitmap.Width, histogramBitmap.Height);
            int yOffset = margin + 3 * oneColorHeight;

            for (int i = 0; i < 256; i++)
            {

                {
                    g.DrawLine(p[3], margin + i, yOffset * (0 + 1), margin + i, yOffset * (0 + 1) - (values[0][i] / maxValues[0]) * 3 * oneColorHeight);
                }
            }
